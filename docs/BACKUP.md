...
4. Tap **Prepare file**, then **Save or share**. On iPhone choose **Save to Files**, then **On My iPhone**; saving under the same name offers **Replace**. On Android choose Files or Drive from the share sheet; recent Chrome can also use a folder you pick once (make a folder such as "Orbit backups", because Android does not allow the top-level Downloads folder). On a computer, Chrome or Edge can use a folder you pick once, and Orbit then replaces the old file and removes older Orbit backups there; other browsers save to Downloads. Orbit only ever writes this file on your device. It does not connect to iCloud, Google Drive or any other service.
