...
**Nothing is copied.** Regoal stores only a preview picture (about 15 KB), the date, your tag and note, and the file's name and size. The original stays in Photos or Files, so your phone does not hold everything twice. What that means where:

- **iPhone (Safari or the Home Screen app):** a web app cannot keep a link into your photo library, so to watch an original again, or to use it in a form check or reel, you pick it again. Regoal uses it in memory and lets go. This is the price of not copying it.
- **Chrome or Edge on a computer, and recent Chrome on Android:** Regoal can keep a real link to each file, so it opens without asking again. If you move or rename the file, the link stops working and you pick it again. If a link cannot be made, Regoal uses the normal file picker instead.
...
A backup includes the entries and their previews, never the originals. Removing an item from Regoal does not touch the photo or video.

**Ask the coach about form.** Open an item and tap it. For a video Regoal takes six still frames spread across it (one for a photo), shows them to you, and names the provider they will go to. Nothing is sent until you tap Send. They go with your own key, are not saved, and can show your face and surroundings. The coach says what it can see and suggests up to four cues; it cannot judge speed or feel, and it can be wrong. You can save the written review with the item.
