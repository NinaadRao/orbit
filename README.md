<p align="center"><img src="icons/icon-192.png" width="96" alt="The Regoal logo: a ring with a green arc and a dot circling a white centre"></p>
<h1 align="center">Regoal</h1>
<p align="center"><b>Track the change. Not the vibes.</b></p>

A private, local-first tracker for lifts, workouts and sport, food, body weight, measurements and progress photos, built around goals you set and review in cycles: a strength plan (build, recomp or cut) of 1 to 24 months, plus runs, rides, swims or any number you want to move. It goes again when a cycle ends. No account, no server, no analytics. Your data lives in your browser and leaves only when you make a backup file. It installs on an iPhone, an Android phone or a computer and works offline.
...
<p align="center">
  ...
  <img src="docs/img/goals.png" width="190" alt="Goals: strength, a half marathon, a faster 5K and pull-ups side by side, each with status, progress and this week's target">
...
<p align="center">
  <img src="docs/img/goal-detail.png" width="190" alt="A goal: status, this week's target, the path you planned against what you did, and every week">
  <img src="docs/img/goal-add.png" width="190" alt="Add a goal: pick a race, set how long it runs, and see the week-by-week path before you save">
  <img src="docs/img/goal-custom.png" width="190" alt="Your own goal: a number you type in, like pull-ups, drawn against a straight line to the target">
  <img src="docs/img/goals-strength.png" width="190" alt="The strength plan as one goal, with weekly targets for as many lifts as you track">
</p>
<p align="center">
  <img src="docs/img/plan-length.png" width="190" alt="Change the plan length any time: 1, 2, 3, 6, 9 or 12 months, or any number of weeks">
...
**Plan**
- Answer a few questions and get calories, macros, a weekly split, week-by-week lift targets with planned deload weeks, and measurement goals. Pick how long the plan runs: 1, 2, 3, 6, 9 or 12 months, or any number of weeks up to 104. Change it later and your history stays.
...
**Goals**
- The Goals tab holds everything you are working towards, and a switcher at the top shows one goal or all of them. Your strength plan is one goal. Add others: a distance (a 10K, a half marathon, a 1 km swim), a pace, a weekly volume for running, cycling or swimming, or any number you type in yourself, like pull-ups or resting heart rate.
- Every goal has its own length, from 1 week to 24 months, and you can edit it, close it, reopen it or delete it. Goals change, so nothing is locked.
- Each endurance goal gets a week-by-week path: a steady build, a lighter week every fourth week, and a taper before the day. This week's target, ahead, on track or behind, and a note when a goal is more ambitious than a gentle climb allows come from rules on your device, no key needed. They are rules of thumb, not a coach.
- When a cycle ends you get a short review and "Start the next cycle" with the next distance and time already suggested. Then you go again.
- Runs, rides and swims count once you give the workout a distance. Notes you write on a goal stay on the device and are never sent to the coach.
...
Then open Regoal from its home screen icon, answer the setup questions (about four minutes), and make your first backup: Today, the shield icon, **Back up now**. Android and iPhone details, updating, and troubleshooting are in [docs/INSTALL.md](docs/INSTALL.md).
...
Regoal bundles about 7,200 foods from USDA FoodData Central. It does **not** bundle the Indian Food Composition Tables 2017 (IFCT), because their publisher, the National Institute of Nutrition (ICMR) in Hyderabad, allows personal use with acknowledgement but not electronic redistribution without written permission. **If you eat Indian food, loading IFCT as your own list is recommended.** It adds 542 foods (dals, millets, vegetables, paneer, eggs, meat and fish) with the diet tag and everyday names filled in. It takes a few minutes and you keep the file to yourself:
...
2. **Convert it.** From this folder: `node scripts/ifct-to-import.mjs package/index.csv --out ~/Documents/orbit-private/ifct.orbitfoods.json`. The script refuses to write inside the Regoal folder, and `*.orbitfoods.json` is ignored by git and blocked by the privacy check.
3. **Load it.** In Regoal: Fuel, **Add food**, **Find**, **Add my own food list**, choose the file, then **Use this list**. Those foods show the list name as their source and follow the Veg / Egg / Non-veg filter. Choose the file again on a new phone; it is not in backups.
...
Optionally list words you never want committed (your name, measurements) in `~/.regoal-private-terms` (the older `~/.orbit-private-terms` still works), one per line. The check reports matches by line number without printing the terms.
